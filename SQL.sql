...
-- Comparison on alphabets: 
  -- dictionary order is followed, that is Ja > J, so when > "J" is in query, it takes all rows, with column value starting with J or bigger
...
-- Computation can be done on column values in a row. To do computation on column values across rows, use the aggregate functions.
